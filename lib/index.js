 ...
 * @param base {String} base s3 path prefix
 * @return {Vinyl} file
 * @api private
 */

function initFile(file, base) {
  if (!file.s3) {
    file.s3 = {};
    file.s3.headers = {};
    file.s3.path = base + file.relative.replace(/\\/g, '/');
  }
  return file;
}
...
Publisher.prototype.publish = function (headers, options) {
  ...
  // init opts
  if (!options) options = { force: false, base:'' };
